...
class SwiftNotice: NSObject {
    ...
    static func showNoticeWithText(_ type: NoticeType,text: String, autoClear: Bool, autoClearTime: Int) -> UIWindow {
        ...
        let labelHeight: CGFloat = label.systemLayoutSizeFitting(CGSize.init(width: 90, height: 80)).height
        label.frame = CGRect(x: 5, y: 60, width: 90, height: labelHeight)

        //textNode.layoutThatFits(ASSizeRange.init(min: CGSize.init(width: 36, height: 16), max: CGSize.init(width: 90, height: 80))).frame.height


        //let labelHeight: CGFloat = 3 + label.systemLayoutSizeFitting(CGSize.init(width: 36, height: 80)).height
        label.frame.size.height = labelHeight
        let frame = CGRect(x: 0, y: 0, width: 100, height: 90 - 16 + labelHeight)
        let window = UIWindow()
        window.backgroundColor = UIColor.clear
        let mainView = UIView()
        ...
        checkmarkView.frame = CGRect(x: 32, y: 15, width: 36, height: 36)
        ...
        mainView.alpha = 0.0
        mainView.transform = CGAffineTransform.init(scaleX: 0.4, y: 0.4 )
        UIView.animate(withDuration: 0.2, animations: {
        ...
        if autoClear {
            self.perform(.hideNotice, with: window, afterDelay: 1.5)
        }
        return window
    }
...
class SwiftNoticeSDK {
    ...
    class var imageOfCheckmark: UIImage {
        ...
        return UIImage.init(named: "toast_checkmark")!.tinted(color: UIColor.white)
    }
    class var imageOfCross: UIImage {
        return UIImage.init(named: "toast_cross")!.tinted(color: UIColor.white)
    }
